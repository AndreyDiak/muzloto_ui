...
/** Забрать награду только за один порог (1, 3 или 5 покупок). */
export async function claimPurchaseReward(
  threshold: number
): Promise<ClaimPurchaseRewardsResult> {
  const {
    data: { session },
  } = await http.auth.getSession();
  if (!session?.access_token) {
  ...
  const backendUrl = (import.meta.env.VITE_BACKEND_URL || "http://localhost:3001").replace(/\/$/, "");
  const res = await fetch(`${backendUrl}/api/achievements/claim-purchase-reward`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${session.access_token}`,
